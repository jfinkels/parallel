...
The queuing shared memory (QSM) machine \cite{gmr97} is a essentially a parameterized QRQW PRAM in which the parameter $g$ scales the cost (of each step in each processor) due to memory accesses.
...
There are many, many other models of parallel computing machines, many of which are likely more accurate for contemporary machines than the QRQW PRAM; see \cite{zcsm07} for details and a listing of modern parallel computation models.
...
\begin{todo}
  Which of the models mentioned in \cite{zcsm07} is closest to our current consumer-grade hardware?
...
\begin{currently}
  ...
  It is similar to the QSM model, but that doesn't account for the XMT memory hierarchy (that is, caches and memory modules).
\end{currently}
