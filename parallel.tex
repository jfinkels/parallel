...
\begin{todo}
  Choose a fixed model of a computation and examine examples of algorithms with respect to these complexity measures.
  Note that simulating a QRQW PRAM on a EREW PRAM can tell us something about tradeoff between computation time and queuing delay.
\end{todo}
...
\section{Highly parallel problems}
...
\section{Inherently sequential problems}
...
\begin{todo}
  What is the relationship between logarithmic space many-one reductions ($\leq_m^L$) and \textsf{NC} many-one reductions ($\leq_m^{NC}$)?
  Since $\mathsf{L}\subseteq\mathsf{NC}$, the former reduction implies the latter.
\end{todo}

\section{Highly parallel appromixations}

An inherently sequential (that is, \textsf{P}-complete) optimization problem may still admit a highly parallel approximation.
Analogous to $\mathsf{P}, \mathsf{APX}, \mathsf{FPTAS}, \mathsf{PTAS}, \mathsf{NPO}$, we have the same classes in \textsf{NC}, and in \textsf{L}.
